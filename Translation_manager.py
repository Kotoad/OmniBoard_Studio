from Imports import json, os, Path, get_Utils, logging
...
class TranslationManager:
    ...
    def __init__(self):
        ...
        if Utils.app_settings.language in self.available_translations:
            #logging.info(f"Loading user preferred language: {Utils.app_settings.language}")
    ...
    def _load_available_translations(self):
        ...
        if not self.translation_dir.exists():
            logging.warning(f"Translation directory {self.translation_dir} does not exist.\nDefaulting to English only.")
            ...
            try:
                with open(json_file, 'r', encoding='utf-8') as f:
                    ...
                    if lang_code and lang_name:
                        ...
                        logging.warning(f"Missing 'main_GUI._metadata.language_name' in {json_file}. Skipping.")
            except json.JSONDecodeError:
                logging.error(f"Failed to parse {json_file}. Skipping.")
            except Exception as e:
                logging.error(f"An error occurred while loading {json_file}: {e}. Skipping.")
    ...
    def _load_translation(self, lang_code):
        ...
        if not translation_file.exists():
            logging.warning(f"Translation file for '{lang_code}' does not exist. Defaulting to English.")
            return False
        ...
        try:
            ...
            logging.error(f"Failed to parse translation file for '{lang_code}'. Defaulting to English.")
            return False
        except Exception as e:
            logging.error(f"An error occurred while loading translation for '{lang_code}': {e}. Defaulting to English.")
            return False
        
    def set_language(self, lang_code):
        if lang_code not in self.available_translations:
            logging.warning(f"Language '{lang_code}' is not available. Available languages: {list(self.available_translations.keys())}")
            return False
    ...
    def translate(self, key, default=None, **kwargs):
        ...
        if text is None:
            text = default or key
            logging.warning(f"Missing translation for key '{key}' in language '{self._current_language}'.")

        if kwargs:
            try:
                text = text.format(**kwargs)
            except KeyError as e:
                logging.warning(f"Missing placeholder {e} in translation for key '{key}'.")
